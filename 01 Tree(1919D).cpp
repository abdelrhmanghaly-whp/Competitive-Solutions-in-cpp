...
bool go(vr &v){
    ...
    for(int i=1; i<v.size(); i++){
        while(cnt && arr[cnt-1] > v[i] && arr[cnt-1]-1==arr[cnt-2])arr.pop_back(),cnt--;
        if(arr[cnt-1]<v[i])arr.pb(v[i]),cnt++;
        else if(arr[cnt-1]-1==v[i])arr[cnt-1]=v[i];
        else if(arr[cnt-1]==v[i]){
            if(v[i]==0)return false;
        }
        else return false;
    }
    while(cnt && arr[cnt-1]-1==arr[cnt-2])arr.pop_back(),cnt--;
    return (cnt==1 && arr[0]==0);
}
void Solution(){
    int n;std::cin>>n;
    vr v(n);
