...
ll dp[MAXN][300],curr[MAXN][300],arr[MAXN];
// offline queries
void Solution(){
    int n,q;std::cin>>n>>q;
    ...
    for(int i=1; i<=n; i++){
        for(int j=1; j<=300; j++){
            if(i-j>0){
                int c=i/j +(i%j!=0);
                dp[i][j]=dp[i-j][j]+arr[i]*c;
                curr[i][j]=curr[i-j][j]+arr[i];
            }
            else dp[i][j]=arr[i],curr[i][j]=arr[i];
        }
    }
    while(q--){
        ll s,d,k;std::cin>>s>>d>>k;
        if(d>=300){
            ll res=0,idx=s;
            for(int i=1; i<=k; i++)res+=arr[idx]*i,idx+=d;
            std::cout<<res<<' ';
        }
        else{
            ll m=s+((k-1)*d);
            ll altercurr=((s/d)-(s%d==0));
            ll res=(dp[m][d]-dp[std::max(0LL,s-d)][d])-std::max(0LL,altercurr)*(curr[m][d]-curr[std::max(0LL,s-d)][d]);
            std::cout<<res<<' ';
        }
    }
    std::cout<<endl
