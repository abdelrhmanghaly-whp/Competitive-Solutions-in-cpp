...
void Solution() {
    ...
    for(int i=1; i<=n; i++){
        for(int j=1; j<=n; j++){
            std::cin>>grid[i][j];
            if(i==1 && j>1)operations[j]=grid[i][j];
            if(i==1 && j==2)operations[1]=grid[i][j];
        }
    }
   // operations[n]=grid[1][n];
    for(int i=1; i<=n; i++){
        for(int j=1; j<=n; j++) {
           // operations[n]&=grid[i][n];
           if(i!=j) operations[j] &= grid[i][j];
        }
    }
    for(int i=1; i<=n; i++){
        for(int j=1; j<=n; j++){
            if(i!=j && grid[i][j]!=(operations[i]|operations[j]))f=0;
        }
    }
    if(f){
        std::cout<<"YES"<<endl
        for(int i=1; i<=n; i++)std::cout<<operations[i]<<' ';
        std::cout<<endl
    }
    else{
        std::cout<<"NO"<<endl
