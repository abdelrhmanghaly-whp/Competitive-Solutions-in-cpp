...
int bfs(int n, int m){
    ...
    vis[n]=1;
    while(!qp.empty()){
        int u=qp.front().ft,dist=qp.front().sd;
        qp.pop();
        if(u==m)return dist;
        if(!vis.count(u-1) && u-1>=1) // min 1 button -->
            qp.push({u-1,dist+1}),vis[u-1]=1;
        if(!vis.count(u*2)&&u<=m)qp.push({u*2,dist+1}),vis[u*2]=1;
    }
}
void Solution(){
    int n,m;std::cin>>n>>m;std::cout<<bfs(n,m)<<endl
}
