...
void Solution(){
    ...
    m[0]=1,m[n-1]=-1;
    for(int i=1; i<n-1; i++){
        if(abs(v[i]-v[i-1])<abs(v[i]-v[i+1]))m[i]=-1;
        else m[i]=1;
    }
    l[0]=0;
    for(int i=1; i<n; i++){
        if(m[i]==-1)l[i]=l[i-1]+1;
        else l[i]=l[i-1]+abs(v[i]-v[i-1]);
    }
    r[n-1]=0;
    for(int i=n-2; i>=0; i--){
        if (m[i] == 1)r[i] = r[i + 1] + 1;
        else r[i] = r[i + 1] + abs(v[i] - v[i + 1]);
    }
    std::cin>>q;
    while(q--){
        ll x,y;std::cin>>x>>y;
        x--,y--;
        if(x==y)std::cout<<0<<endl
        else{
            if(y>x)std::cout<<r[x]-r[y]<<endl
            else std::cout<<l[x]-l[y]<<endl
        }
    }
