...
void Solution(){
    ...
    int res=0;
        ...
        fj(j,v[i].size()-1){
            ...
            //if(next==n)next=0;
            //if(prev==-1)prev=n-1;
            int cnt1= std::lower_bound(v[next].begin(),v[next].end(),v[i][j])-v[next].begin();
            int cnt2= std::lower_bound(v[next].begin(),v[next].end(),v[i][j+1])-v[next].begin();
            int cnt3= std::lower_bound(v[prev].begin(),v[prev].end(),v[i][j])-v[prev].begin();
            int cnt4= std::lower_bound(v[prev].begin(),v[prev].end(),v[i][j+1])-v[prev].begin();
            if(cnt4-cnt3!=cnt2-cnt1)res++;
        }
    }
    std::cout<<res<<endl
