...
void Solution(){
    ll n,q;std::cin>>n>>q;
    vrl v(n),curr(n+1),pref(n+1);
    in(v);
    fi(i,n){
        if(v[i]==1)curr[i+1]=curr[i]+1;
        else curr[i+1]=curr[i];
    }
    for(int i=1; i<=n; i++)pref[i]=pref[i-1]+v[i-1]-1;
    while(q--){
        ll l,r;std::cin>>l>>r;
        if(l==r){
            std::cout<<"NO"<<endl
        ...
        ll sum=pref[r]-pref[l-1],ans=curr[r]-curr[l-1];
        if(sum>=ans)std::cout<<"YES"<<endl
